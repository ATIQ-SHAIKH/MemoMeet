    ...
    try {
        ...
        if (!user) {
            const newUser = await userManager.create({
                ...
                email,
                password,
                email_verification_info: { // setting because this is to be used for MVP
                    is_verified: true,
                    verification_email_sent_on: new Date(),
                    verification_token: "verificationToken",
                    verification_token_expires_at: new Date(new Date().getTime() + 24 * 60 * 60 * 1000)
                }
            });
            if (!newUser) return res.status(RESPONSES.INTERNAL_SERVER_ERROR).json({ msg: GENERAL_MESSAGES.SOME_UNKNOWN_ERROR_OCCURED })
            const token = await jwtSign({ _id: newUser._id, name: newUser.name, email });
            ...
            return res.json({ msg: USER_LOGIN_MESSAGES.LOGGED_IN });
        ...
        const token = await jwtSign({ _id: user._id, name: user.name, email });
...
module.exports = {
    // signupInvite, signupSetPassword, login
    signin,
    checkSession,
    logout
};